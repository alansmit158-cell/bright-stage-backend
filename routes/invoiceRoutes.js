...
router.post('/', protect, authorize('Founder', 'Manager', 'Admin'), async (req, res) => {
    try {
        ...
        const newInvoice = new Invoice({
            ...req.body,
            createdBy: (/** @type {any} */ (req)).user._id
        });
...
router.post('/from-project/:projectId', protect, authorize('Founder', 'Manager', 'Admin'), async (req, res) => {
    try {
        const project = await Project.findById(req.params.projectId).populate('clientId').populate('items.inventoryItem');
        ...
        const clientData = project.clientId || project.client || {};

        const newInvoice = new Invoice({
            number,
            client: {
                id: clientData._id || project.clientId,
...
router.get('/:id/pdf', async (req, res) => {
    try {
        ...
        PdfService.generateInvoiceFromModel(invoice, res, companyId, settings);
