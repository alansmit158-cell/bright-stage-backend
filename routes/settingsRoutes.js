...
router.get('/company', async (req, res) => {
    try {
        const key = req.query.key || 'bright';
        let settings = await CompanySettings.findOne({ companyKey: key });
        ...
        if (!settings) {
            if (key === 'bright') {
                ...
                settings = new CompanySettings({
                    companyKey: 'square',
                    companyName: 'Square Event',
                    address: {
                        street: 'Rue UMA la Soukra',
                        city: 'Ariana Tunis',
                        postalCode: '2036',
                        country: 'Tunisia'
                    },
                ...
                return res.status(404).json({ error: 'Settings for this company not found' });
            }
            await settings.save();
        }
...
router.put('/company', protect, authorize('Founder', 'Admin'), async (req, res) => {
    try {
        const key = req.body.companyKey || req.query.key;
        if (!key) return res.status(400).json({ error: 'Company key required' });

        let settings = await CompanySettings.findOne({ companyKey: key });

        if (!settings) {
            settings = new CompanySettings({ ...req.body, companyKey: key });
        } else {
            // Sanitize: remove fields that should not be updated via body
            const { _id, __v, companyKey, ...updateData } = req.body;
            Object.assign(settings, updateData);
            settings.companyKey = key; // Ensure key remains correct
        }
